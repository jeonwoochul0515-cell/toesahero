...
## 🆕 기능 고도화 관련 액션 (FEATURE_PLAN.md) — 하나씩 진행

> 일본 레퍼런스 기반 고도화에서 변호사님 입력이 필요한 항목. 권장 순서 **K → L → M**, 그리고 어드민/판단 항목 N·O.

---

## K. Firebase Storage 초기화 (증거 업로드 활성화, P1-2) 🔴 즉시

기능: 의뢰인이 사건에 증거 파일(녹취·캡처·계약서)을 안전 업로드. 코드·보안규칙 준비 완료 — Storage만 켜면 활성화.

### Co-work 프롬프트
```
Firebase 콘솔에서 Cloud Storage를 처음 설정하려고 합니다. 구글 계정 로그인은 화면에서 제가 직접 하겠습니다.

순서:
1. https://console.firebase.google.com/project/durable-binder-457823-g3/storage 접속
2. (로그인 필요 시 본인이 직접 로그인)
3. "시작하기"(Get started) 클릭
4. 보안 규칙 다이얼로그 → "프로덕션 모드에서 시작" 선택 → 다음
   (규칙은 개발자가 storage.rules로 덮어쓰므로 무엇을 골라도 무방)
5. 위치(location) → asia-northeast3 (서울) 선택 → 완료
6. "파일" 탭이 보이고 버킷이 생성되면 성공. 그 화면을 캡처해 주세요.

받아올 키 없음. "버킷 생성 완료"만 확인되면 됩니다.
```
### 완료 후
Claude Code에 "Storage 켰어"라고 알려주시면 → `firebase deploy --only storage` 실행 → 업로드 즉시 활성화·검증.

---

## L. 토스 간편결제·무이자할부 활성화 (P0-3) 🟡

기능: 결제창에 카카오페이·네이버페이·무이자할부 노출. **코드 변경 불필요** — 상점관리자 설정만.

### Co-work 프롬프트
```
토스페이먼츠 상점관리자에서 간편결제·무이자할부를 활성화하려 합니다. 로그인은 화면에서 제가 직접 하겠습니다.

순서:
1. https://merchant.tosspayments.com 접속 → (본인이 직접 로그인)
2. 좌측 "결제수단"(또는 결제수단 관리) 메뉴
3. 간편결제에서 신청/사용 설정:
   - 토스페이 (보통 기본 제공)
   - 카카오페이 → 신청/사용 (약관 동의·심사 있을 수 있음)
   - 네이버페이 → 동일
4. "무이자 할부" 메뉴에서 카드사·개월 설정 확인
5. 각 수단의 상태(사용중/신청중/미신청)를 캡처해 주세요.

카카오·네이버페이는 심사 며칠 걸릴 수 있음 — "신청 접수"까지만 해도 됨.
코드 작업 없음. 활성화되면 기존 결제창에 자동 노출됩니다.
```
### 완료 후
상태만 알려주시면 됩니다(코드 작업 없음). 결제창에서 노출 확인.

---

## M. 카카오 알림톡 — 진행상황 알림 (P1-1) 🔵 심사 1~3일

기능: 사건 단계가 바뀌면 의뢰인에게 카카오 알림톡 발송. **현재 문자 발송사는 SOLAPI**(`api.solapi.com`)이므로 알림톡도 SOLAPI로 통일. ⚠️ 기존 액션 I(NHN Cloud)는 실제 코드와 다르니 **SOLAPI로 진행**.

### Co-work 프롬프트
```
SOLAPI(https://solapi.com)에서 카카오 알림톡을 설정하려 합니다. 로그인·사업자 인증은 화면에서 제가 직접 하겠습니다.

순서:
1. https://solapi.com 로그인 → 콘솔
2. "카카오 알림톡" 메뉴 → "카카오 비즈니스 채널 연동"
   (퇴사히어로 카카오 채널을 비즈니스 채널로 등록·연동)
3. "발신프로필" 등록 → 생성된 발신프로필 키(pfId) 확인
4. "알림톡 템플릿" → 새 템플릿 등록, 아래 내용으로 신청:
   - 템플릿명: 사건진행알림
   - 내용:
     [퇴사히어로] 사건 진행 안내
     #{name}님 사건이 '#{stage}' 단계로 업데이트되었습니다.
     진행 확인: https://toesahero.com/my
     문의: 1660-4452
   - 변수: #{name}, #{stage}
5. 심사 통과 후 발신프로필 키(pfId)와 템플릿 ID(코드)를 화면에 보여주세요.

* 발신프로필 키·템플릿 ID만 알려주시면 됩니다. SOLAPI API Key/Secret은 채팅에 적지 마세요.
```
### 완료 후
발신프로필 키 + 템플릿 ID 알려주시면 → `/api/notify`에 알림톡 분기 연결 + 어드민 상태변경 시 자동 발송.

---

## N. 실적/신뢰 지표 입력 (P2-3) — 어드민 (브라우저 자동화 아님)
1. https://toesahero.pages.dev/admin/login 로그인
2. 상단 "실적 지표"(/admin/stats) 메뉴
3. **실제로 사실인 값만** 입력(허위·과장 금지). 예: "누적 상담 / 1,200+건", "변호사 경력 / 10년+", "대한변협 / 정식 등록". 저장 시 홈 상단 자동 노출.

## O. 성공보수 옵션 결정 (P0-4) — 변호사 판단
구현 전 아래만 정해주시면 됩니다(임금·퇴직금 회수 사안 한정, 형사 제외).
- 착수금 금액 (예: 표준 사안 15만 원)
- 성공보수율 — 회수액의 몇 %? (예: 15~25%)
- 적용 범위: 미지급 임금·퇴직금 회수(민사·노동청)만. 형사고소는 성공보수 무효(대법원).
- 보수약정서 문구: 개발자가 초안 제공 → 변호사 검토·확정.
