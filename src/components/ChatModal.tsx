...
const fallbackResponses: Record<string, string> = {
  ...
  "비용이 궁금해요":
    "단순 통보 199,000원, 임금 청구 통합 390,000원, 분쟁 대응 790,000원 — 세 가지 정액 패키지이며 모두 부가세가 포함된 금액입니다. 상황을 말씀해 주시면 맞는 패키지를 안내드립니다.",
  "계약서에 무서운 조항이 있어요":
...
export function ChatModal({ open, onClose, greeting }: Props) {
  ...
  const submitContact = async () => {
    ...
    setMessages((m) => [
      ...
      {
        who: "them",
        text: `${name}님, 연락처를 전달했습니다. 변호사 ${REVIEWING_LAWYER}가 영업일 기준 24시간 이내에 ${phone} 로 연락드리겠습니다.`,
        expression: "cheer",
      },
    ]);
