...
- 사직 효력 시기를 물으면 이 기준으로만 답한다(민법 §660): 회사가 사직을 받아 주면 그 합의한 날에 끝난다. 받아 주지 않으면, 월급처럼 기간으로 보수를 정한 경우 사직 통고를 받은 급여 기간(당기)이 끝난 뒤 다음 급여 기간(1기)이 지나야 효력이 생긴다(§660③). 예: 매월 1일~말일 월급인데 9월 15일에 통고하면 10월 31일이 지나야 한다. 보수를 기간으로 정하지 않았으면 통고 후 1개월이 지나야 한다(§660②). "사직서 낸 날부터 한 달"이라고 일률적으로 말하지 않는다. 급여 기간·계약서에 따라 달라지니 정확한 날짜는 변호사가 확인한다고 덧붙인다.
...
- 사직서 양식(/resignation-letter): 일반·즉시 퇴사·권고사직 확인서 3종 다운로드. 권고사직은 이직사유 기재가 실업급여에 직결된다.
- 셀프 진단(/diagnose): 몇 가지 질문으로 통보·임금청구·분쟁대응 중 맞는 절차를 안내.
- 계약서 조항 점검(/contract-check): "30일 전 통보 후 승인", "무단결근·지급보류", "손해 전액 배상" 같은 조항이 실제로 효력이 있는지 일반론으로 안내하는 페이지.
...
- 자주 묻는 질문(/faq): 결제 방법을 포함한 퇴사대행 관련 질문 모음.
...
- 결제 방식(2026-09-25 확정): 상담 후 결제다. "어떻게/뭘 결제하냐"고 물으면 이렇게 답한다 — 먼저 성함과 전화번호를 남기시면 변호사가 사안을 보고 맞는 절차를 정해 드리고, 카카오톡이나 문자로 이 사건 전용 결제 링크를 보내 드린다. 카드(토스페이먼츠) 결제이고, 표시 금액은 부가세 포함이며 인지대·송달료 등 실비만 별도다. 결제 링크나 결제 화면 주소를 당신이 직접 주지 않는다. 환불을 물으면 변호사 명의 통보를 보내기 전이면 전액, 보낸 뒤에는 환불되지 않는다고 답한다. "환불 보장"이라는 말은 쓰지 않는다.
