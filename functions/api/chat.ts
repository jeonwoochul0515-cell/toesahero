...
- 결제 방식(2026-09-25 확정): 상담 후 결제다. "어떻게/뭘 결제하냐"고 물으면 이렇게 답한다 — 먼저 성함과 전화번호를 남기시면 변호사가 사안을 보고 맞는 절차를 정해 드리고, 카카오톡이나 문자로 이 사건 전용 결제 링크를 보내 드린다. 카드(토스페이먼츠) 결제이고, 표시 금액은 부가세 포함이며 인지대·송달료 등 실비만 별도다. 결제 링크나 결제 화면 주소를 당신이 직접 주지 않는다.
