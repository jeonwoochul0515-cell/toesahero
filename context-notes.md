...
### 롱테일 1,100개 대량 등록 (같은 날 저녁, 사용자 플랜 실행)
사용자 플랜: "세부 교차검색어를 70원 초저가로 1,000개 이상 등록해 돈 아끼고 유입 늘리기".

- **검증됨**: 시드 49개 → 연관어 4,241개 → 관련성 필터 1,310개 → 전수 입찰가 조회 결과 **1,105개가 70~90원으로 5위 노출 가능**. 검색량 30~99회 구간은 92%가 70원이면 충분.
- **플랜의 전제 하나는 틀림**: 유입 상한은 키워드 수가 아니라 **예산**(5,000원 ÷ 70원 = 하루 71클릭 천장). 키워드 확대의 진짜 가치는 "기존 20개로는 예산을 다 쓰지도 못했다"는 점 — 유입 증대가 아니라 **예산 활용률** 개선.
- **관련성 필터 필수**: 네이버 연관어는 주제를 크게 벗어남(주얼리모델·하객알바·호텔예약확인서 등 **69%가 무관**). 그대로 등록하면 품질지수 하락 + 무의미한 클릭 비용.
- **의도 불균형**: 정보성 검색량이 의뢰 의도의 **24배**(78.7만 vs 3.3만회). 섞으면 하루 71클릭이 전부 정보성으로 소진 → **사용자 결정으로 광고그룹 2개 분리**.
- **최종 구조**: 상담그룹 `최고신뢰8개`(273개, 일예산 5,000원, 소재→홈) / 도구그룹 `롱테일_도구정보`(`grp-...-070458075`, 835개, 일예산 2,000원, 소재→`/calc`). 도구 키워드 랜딩은 사용자 결정대로 `/calc` 71개, `/resignation-letter` 33개, `/unemployment-calc` 4개 개별 지정.
- 최종 후보 목록은 **`docs/ads_keyword_candidates.csv`**(1,105행)에 보존. 스크래치패드 스크립트는 세션 종료 시 소멸하므로 재작업 시 CSV부터 볼 것.

**치명적 API 함정 (전역 CLAUDE.md에도 기록)**: `POST /ncc/keywords`는 **실패를 HTTP 200 안에 숨긴다**. 838건 "성공"으로 집계했는데 실제 727건만 등록됨 — 항목별 `resultStatus.code`를 검사해야 하고 `len(response)`를 성공 수로 세면 안 된다. 누락 원인은 `links` 형식(평평한 문자열 ✗ → `{"pc":{"final":url}}` 중첩 ✓). 소재는 헤드라인 가운뎃점(`·`)에서 `1030` 오류.

### 2~3차 확장 — 최종 1,411개 (같은 날, "키워드 더 늘려줘")
- **2차**: 신규 주제(육아휴직·근로시간·4대보험·징계·산재·특수고용) 시드 63개 + 기등록 상위 60개 연쇄 재투입 → 10,663개 수집(신규 7,228). 필터 후 665개 → 100원 이하 555개 확정 → 상담 100 + 도구 165 등록.
- **광고그룹 한도 확정**: 도구그룹이 1,000개에서 막힘(`3911`). **광고그룹당 정확히 1,000개**. 나머지 286개 거부됨.
- **3차**: 상담그룹 여유(589칸)를 의뢰 의도로 채우려 전용 시드 40개 수집(1,208개). **타 법률분야 오염 심각** — 통매음합의금·명도합의서·위자료청구·나의변호사 등. 노동 도메인 단어 AND 조건 강제 → 183개→40개, 38개 등록(2개 금지 키워드).
- **교훈**: 의뢰 의도 시드("변호사·소송·합의서")는 범용 법률어라 다른 분야를 물어온다. 도메인 단어를 AND로 강제할 것.
- **최종**: 상담그룹 411개(여유 589) / 도구그룹 1,000개(포화) / 합계 **1,411개**. 전체 현황 CSV = `docs/ads_registered_keywords.csv`.
- **의도적 미등록**: 2차 잔여 290개는 월검색 합계 8,080회뿐이고 품질 낮음(육군징계규정·간호사직업윤리·COPD산재). 새 그룹+소재검수 감수할 가치 없다고 판단.
- **여기서부터 수익 급감**: 남은 레버는 ①예산 증액(71클릭 천장이 진짜 병목) ②입찰가 인상(대형 키워드 개방, 단가 20~40배) ③전환율 개선. **셋 다 키워드 추가가 아니다** — 키워드를 더 늘려달라는 요청이 또 오면 이 점을 먼저 설명할 것.

## 다음 세션
- **2주 뒤(2026-08-03경) 성과 점검**: `GET /stats`로 노출·클릭·소진액 확인. 판단 기준 4가지는 `docs/ADS_PLAYBOOK.md` 측정 절 참조. 특히 `권고사직위로금` 소진 비중과 **그룹별 소진액 균형** 확인.
- **핵심 가설 검증**: "계산기 쓰러 온 사람이 미지급 항목을 발견하면 상담으로 이어진다". 도구그룹 클릭 대비 상담 증가가 전혀 없으면 도구그룹 예산 2,000원을 상담그룹으로 옮길 것.
